...
import { NutritionTableRow } from '../components';
// ----------------------------------------------------------------------
...
const DEFAULT_QUERY = { page: 1, limit: Number(limit), type: 'nutrition' };

export default function NutritionServiceList() {
  const {
    dense,
    order,
    orderBy,
  ...
  const { isLoading, ServiceData, totalCount } = useSelector((store) => store?.service);
  ...
  const handleEditRow = (row) => {
    navigate(PATH_DASHBOARD.nutriservices.edit(row?._id), { state: row });
  };

  const handleViewRow = (row) => {
    navigate(PATH_DASHBOARD.nutriservices.view(row?._id), { state: row });
  };
  ...
  return (
    <>
      <Helmet>
        <title> Nutrition Services: List | VHAI </title>
      </Helmet>

      <Container maxWidth={themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          heading="Nutrition Services List"
          links={[
            { name: 'Dashboard', href: PATH_DASHBOARD.root },
            { name: 'Nutrition Services', href: PATH_DASHBOARD.nutriservices.list },
            { name: 'List' },
          ]}
          action={
            <Button
              component={RouterLink}
              to={PATH_DASHBOARD.nutriservices.new}
              variant="contained"
...
function applyFilter({ inputData, comparator, filterName, filterStatus, filterRole }) {
  ...
  if (filterName) {
    inputData = inputData.filter(
      (user) => user.roleName.toLowerCase().startsWith(filterName.toLowerCase())
    );
  }
