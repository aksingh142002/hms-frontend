...
import { getStaffNutAsync, putAssignStaffAsync } from '@redux/services';
...
AssignNutritionistForm.propTypes = {
  isView: PropTypes.bool,
  currentAssign: PropTypes.object,
};

export default function AssignNutritionistForm({ isView = false, currentAssign }) {
  const navigate = useNavigate();

  const dispatch = useDispatch();

  const { enqueueSnackbar } = useSnackbar();
  const { staffNut } = useSelector((store) => store?.staff);

  const UserSchema = Yup.object().shape({
    staff: Yup.object().required('Assign Nutritionist is required.'),
  });
  ...
  const onSubmit = async (data) => {
    const payload = {
      assignNutritionist: data?.staff?._id,
