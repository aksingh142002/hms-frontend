...
OtherDetails.propTypes = {
  ...
  setOtherDetailsData: PropTypes.array,
  otherDetailsData: PropTypes.array,
};
...
export default function OtherDetails({
  ...
  useEffect(() => {
    ...
    setOtherDetailsData(currentBook || []);
  }, [currentBook, setOtherDetailsData]);

  const handleOpenPopover = (event, index) => {
    setOpenPopover(event.currentTarget);
    setTableId(index);
    setFillForm(otherDetailsData[index]);
  };
  ...
  const handleEdit = (index) => {
    const selectedData = otherDetailsData[index];
  ...
  const onSubmit = async (data) => {
    ...
    if (formEdit) {
      ...
      const updatedArray = otherDetailsData.map((item, index) => (index === tabelId ? data : item));
      setOtherDetailsData(updatedArray);
      setFormEdit(false);
    } else {
      // If not in edit mode, add a new object to the array
      setOtherDetailsData([...otherDetailsData, newData]);
      setFormEdit(false);
    }
  ...
  const removeData = (id) => {
    const updatedArray = otherDetailsData.filter((data, index) => index !== tabelId);
    setOtherDetailsData(updatedArray);
    handleCloseConfirm();
  };
            ...
            <Card sx={{ p: 3, mt: 3 }}>
              ...
              <TableContainer component={Paper}>
                <Table sx={{ minWidth: 650, mt: 3 }} aria-label="caption table">
                  ...
                  <TableBody>
                    {otherDetailsData?.map((row, index) => (
