...
export default function LeaveForm({ isEdit = false, isView = false, currentLeave }) {
  ...
  const defaultValues = useMemo(
    () => ({
      staff: currentLeave?.staffId || null,
  ...
  useEffect(() => {
    if (isEdit && currentLeave || isView && currentLeave) {
      reset(defaultValues)
      // If it's an edit or view case, populate daysTotal with Leave data
      const initialDaysTotal =
        currentLeave?.leaves?.map((leave) => ({
          date: new Date(leave.date),
          availability: leave.availability,
          slots: leave.slots.map((slot) => ({
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (response?.payload?.success) {
        enqueueSnackbar(
          isEdit ? 'Leave Update successfully!' : 'Leave Created successfully!'
        );
